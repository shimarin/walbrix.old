...
void setup(inifile_t ini)
{
  ...
  if (persistent) {
    const char* datafile = "/mnt/boot/efi/boot/bootx86.dat";
    if (!exists(datafile)) {
      if (get_free_disk_space("/mnt/boot") >= (uint64_t)1024*1024*1024*2 ) {
  ...
  if (swap) {
    const char* swapfile = "/newroot/run/initramfs/boot/efi/boot/bootx86.swp";
    if (!exists(swapfile)) {
      if (get_free_disk_space("/newroot/run/initramfs/boot") >= (uint64_t)1024*1024*1024*2 ) {
...
void init()
{
  ...
  mount_ro_loop_or_die("/mnt/boot/efi/boot/bootx86.efi", "/mnt/system", 1048576);

  process_inifile("/mnt/boot/efi/boot/bootx86.ini", setup);
